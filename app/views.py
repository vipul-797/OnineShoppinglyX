...
def topwear(request, data=None):
  if data == None:
    topwears = Product.objects.filter(category='TW')
  elif data =='Lymio' or data == 'MOLISHA':
...
def bottomwear(request, data=None):
  if data == None:
    bottomwears = Product.objects.filter(category='BW')
  elif data =='Tavasya' or data == 'Jockey':
    bottomwears = Product.objects.filter(category='BW').filter(brand=data)
  elif data == 'below':
    bottomwears = Product.objects.filter(category='BW').filter(discounted_price__lt=190)
  elif data == 'above':
    bottomwears = Product.objects.filter(category='BW').filter(discounted_price__gt=190)
...
def checkout1(request):
  if request.method == 'POST':
        ...
        client = razorpay.Client(
          auth=('rzp_test_KnZp6TqJVVtosn', 'in4ht7HNhJyhAbIEIg7ryux1'))
